...
def check_for_unmanaged_sharing(model: Module, raise_error: bool = True) -> dict[str, list[str]] | None:
    """
    Check if a model has shared parameters that are NOT managed by ShareModule.

    This function detects when Parameter objects are shared (same Python object
    assigned to multiple attributes) but the model isn't wrapped in ShareModule.
    This is a common mistake that leads to silent bugs where parameter updates
    don't propagate correctly.

    Args:
        model: The model to check. Can be any equinox Module.
        raise_error: If True (default), raises ValueError when unmanaged sharing
            is detected. If False, returns the sharing info without raising.

    Returns:
        If raise_error=False and sharing is detected, returns a dict mapping
        parent parameter paths to lists of duplicate paths (like get_sharing_summary).
        Returns None if no sharing detected or if model is already a ShareModule.

    Raises:
        ValueError: If unmanaged sharing is detected and raise_error=True.

    Example:
        >>> # Accidentally created shared parameters without build_model
        >>> class MyModel(Module):
        ...     param_a: Parameter
        ...     param_b: Parameter  # Same object as param_a!
        ...     def __init__(self):
        ...         shared = Parameter(initial=1.0)
        ...         self.param_a = shared
        ...         self.param_b = shared
        >>> model = MyModel()
        >>> check_for_unmanaged_sharing(model)  # Raises ValueError!

        >>> # Correct usage
        >>> model = build_model(MyModel)  # Wraps in ShareModule
        >>> check_for_unmanaged_sharing(model)  # Returns None, all good
    """
    # If already wrapped in ShareModule, sharing is managed
    if isinstance(model, ShareModule):
        return None
    ...
    # If no sharing detected, we're good
    if not dupl_ids:
        return None

    # Build a summary of the sharing
    from collections import defaultdict
    summary: dict[str, list[str]] = defaultdict(list)
    for dupl_id, dupl_path in zip(dupl_ids, dupl_paths):
        parent_path = parent_paths[dupl_id]
        parent_path_str = leafpath_to_str(parent_path)
        dupl_path_str = leafpath_to_str(dupl_path)
        summary[parent_path_str].append(dupl_path_str)

    sharing_dict = dict(summary)

    if raise_error:
        # Format a helpful error message
        sharing_lines = []
        for parent_path, dupl_paths_list in sharing_dict.items():
            # Strip .val/.unconstrained_val suffix for cleaner display
            parent_display = parent_path.rsplit(".", 1)[0] if "." in parent_path else parent_path
            for dupl_path in dupl_paths_list:
                dupl_display = dupl_path.rsplit(".", 1)[0] if "." in dupl_path else dupl_path
                sharing_lines.append(f"  {parent_display}  ←  {dupl_display}")

        raise ValueError(
            f"Model has shared parameters that are not managed by ShareModule!\n\n"
            f"Detected sharing:\n"
            + "\n".join(sharing_lines) + "\n\n"
            f"This will cause bugs where parameter updates don't propagate correctly.\n"
